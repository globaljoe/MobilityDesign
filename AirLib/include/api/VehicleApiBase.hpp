...
#include "sensors/gpu_sonar/GpuSonarBase.hpp"
#include "sensors/gpu_sidescan_sonar/GpuSidescanSonarBase.hpp"
#include "sensors/imu/ImuBase.hpp"
#include "sensors/barometer/BarometerBase.hpp"
#include "sensors/pressure/PressureBase.hpp"
...
*/
    ...
    public:
        ...
        */
        ...
        // GPU Sonar APIs
        virtual const GpuSonarData& getGpuSonarData(const std::string& sonar_name) const
        {
            auto* sonar = static_cast<const GpuSonarBase*>(findSensorByName(sonar_name, SensorBase::SensorType::GpuSonar));
            if (sonar == nullptr)
        ...
        // GPU Sidescan Sonar APIs
        virtual const GpuSidescanSonarData& getGpuSidescanSonarData(const std::string& sonar_name) const
        {
            auto* sidescan_sonar = static_cast<const GpuSidescanSonarBase*>(findSensorByName(sonar_name, SensorBase::SensorType::GpuSidescanSonar));
            if (sidescan_sonar == nullptr)
                throw VehicleControllerException(Utils::stringf("No sidescan sonar with name %s exist on vehicle", sonar_name.c_str()));

            return sidescan_sonar->getOutput();
        }
        ...
        // Pressure API
        virtual const PressureBase::Output& getPressureData(const std::string& pressure_name) const
        {
            auto* pressure = static_cast<const PressureBase*>(findSensorByName(pressure_name, SensorBase::SensorType::Pressure));
            if (pressure == nullptr)
                throw VehicleControllerException(Utils::stringf("No pressure sensor with name %s exist on vehicle", pressure_name.c_str()));

            return pressure->getOutput();
        }

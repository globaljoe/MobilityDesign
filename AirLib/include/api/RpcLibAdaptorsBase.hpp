...
namespace airlib_rpclib
{
    ...
    public:
        ...
        struct CollisionInfo
        {
            ...
            MSGPACK_DEFINE_ARRAY(has_collided, normal, impact_point, position, penetration_depth, time_stamp, object_name, object_id);
        ...
        //     std::vector<msr::airlib::TTimePoint> time_stamps;

        //     MSGPACK_DEFINE_ARRAY(poses, time_stamps);

        //     Trajectory()
        //     {
        //     }
        //     Trajectory(const std::vector<msr::airlib::Pose>& p,
        //                const std::vector<msr::airlib::TTimePoint>& t)
        //     {
        ...
        struct GpuSonarData
        {
            ...
            msr::airlib::TTimePoint time_stamp; // timestamp
            msr::airlib::real_T min_range; //m
            msr::airlib::real_T max_range; //m
            std::vector<float> sonar_raw_data; // sonar data
            std::vector<int> segmentation_sonar;
            std::vector<float> point_cloud; // pointcloud
            std::vector<int> segmentation;
            Pose pose;

            MSGPACK_DEFINE_ARRAY(time_stamp, min_range, max_range, sonar_raw_data, segmentation_sonar, point_cloud, segmentation, pose);

            GpuSonarData()
            {
            }

            GpuSonarData(const msr::airlib::GpuSonarData& s)
            {
                time_stamp = s.time_stamp;
                min_range = s.min_range;
                max_range = s.max_range;
                sonar_raw_data = s.sonar_raw_data;
                segmentation_sonar = s.segmentation_sonar;
                point_cloud = s.point_cloud;
                segmentation = s.segmentation;
                pose = s.pose;
            }

            msr::airlib::GpuSonarData to() const
            {
                msr::airlib::GpuSonarData d;

                d.time_stamp = time_stamp;
                d.min_range = min_range;
                d.max_range = max_range;
                d.sonar_raw_data = sonar_raw_data;
                d.segmentation_sonar = segmentation_sonar;
                d.point_cloud = point_cloud;
                d.segmentation = segmentation;
                d.pose = pose.to();
        ...
        struct GpuSidescanSonarData
        {

            msr::airlib::TTimePoint time_stamp; // timestamp
            msr::airlib::real_T min_range; //m
            msr::airlib::real_T max_range; //m
            std::vector<float> port_raw_data;
            std::vector<int> port_segmentation;
            std::vector<float> port_point_cloud; // pointcloud
            std::vector<int> port_segmentation_cloud;

            std::vector<float> starboard_raw_data;
            std::vector<int> starboard_segmentation;
            std::vector<float> starboard_point_cloud; // pointcloud
            std::vector<int> starboard_segmentation_cloud;
            Pose pose;

            MSGPACK_DEFINE_ARRAY(time_stamp, min_range, max_range, port_raw_data, starboard_raw_data, port_segmentation, starboard_segmentation, port_point_cloud, starboard_point_cloud, port_segmentation_cloud, starboard_segmentation_cloud, pose);

            GpuSidescanSonarData()
            {
            }

            GpuSidescanSonarData(const msr::airlib::GpuSidescanSonarData& s)
            {
                time_stamp = s.time_stamp;
                min_range = s.min_range;
                max_range = s.max_range;
                port_raw_data = s.port_raw_data;
                port_segmentation = s.port_segmentation;
                port_point_cloud = s.port_point_cloud;
                port_segmentation_cloud = s.port_segmentation_cloud;

                starboard_raw_data = s.starboard_raw_data;
                starboard_segmentation = s.starboard_segmentation;
                starboard_point_cloud = s.starboard_point_cloud;
                starboard_segmentation_cloud = s.starboard_segmentation_cloud;

                pose = s.pose;
            }

            msr::airlib::GpuSidescanSonarData to() const
            {
                msr::airlib::GpuSidescanSonarData d;

                d.time_stamp = time_stamp;
                d.min_range = min_range;
                d.max_range = max_range;
                d.port_raw_data = port_raw_data;
                d.port_segmentation = port_segmentation;
                d.port_point_cloud = port_point_cloud;
                d.port_segmentation_cloud = port_segmentation_cloud;

                d.starboard_raw_data = starboard_raw_data;
                d.starboard_segmentation = starboard_segmentation;
                d.starboard_point_cloud = starboard_point_cloud;
                d.starboard_segmentation_cloud = starboard_segmentation_cloud;
                d.pose = pose.to();
        ...
        {
            msr::airlib::TTimePoint time_stamp;
            msr::airlib::real_T depth;
            msr::airlib::real_T pressure;

            MSGPACK_DEFINE_ARRAY(time_stamp, depth, pressure);

            PressureData()
            {
            }

            PressureData(const msr::airlib::PressureBase::Output& s)
            {
                time_stamp = s.time_stamp;
                depth = s.depth;
                pressure = s.pressure;
            ...
            msr::airlib::PressureBase::Output to() const
            {
                msr::airlib::PressureBase::Output d;
        ...
        {
            msr::airlib::TTimePoint time_stamp;
            Vector3r velocity; //[m/s]
            Vector3r angular_velocity; //[rad/s]
            Pose estimated_pose;
            msr::airlib::real_T altitude; //meters
            msr::airlib::real_T min_distance; //m
            msr::airlib::real_T max_distance; //m
            Pose relative_pose;
            ...
            DvlData(const msr::airlib::DvlData& s)
            {
                ...
                velocity = s.velocity;
                angular_velocity = s.angular_velocity;
                estimated_pose = s.estimated_pose;
                altitude = s.altitude;
                min_distance = s.min_distance;
                max_distance = s.max_distance;
                relative_pose = s.relative_pose;
            ...
            msr::airlib::DvlData to() const
            {
                ...
                d.velocity = velocity.to();
                d.angular_velocity = angular_velocity.to();
                d.estimated_pose = estimated_pose.to();
                d.altitude = altitude;
                d.min_distance = min_distance;
                d.max_distance = max_distance;
                d.relative_pose = relative_pose.to();

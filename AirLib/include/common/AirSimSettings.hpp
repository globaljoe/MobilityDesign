...
namespace airlib
{
    ...
    public: //types
        ...
        static constexpr char const* kVehicleTypeAsvSimple = "asvsimple";
        static constexpr char const* kVehicleTypeArduRover = "ardurover";
        static constexpr char const* kVehicleTypeAuvSimple = "auvsimple";
        ...
        static constexpr char const* kSimModeTypeAsv = "Asv";
        static constexpr char const* kSimModeTypeAuv = "Auv";
        ...
        struct CaptureSetting
        {
            ...
            float fov_degrees = 70.2f; // Utils::nan<float>(); //90.0f
            float fov_vertical_degrees = Utils::nan<float>(); // when set, enables anamorphic (independent H/V) projection
        ...
        struct PressureSetting : SensorSetting
        {
        };
        ...
        struct GpuSonarSetting : SensorSetting
        {
            enum class DataFrame
            {
                VehicleInertialFrame,
        ...
        struct GpuSidescanSonarSetting : SensorSetting
        {
            enum class DataFrame
            {
                VehicleInertialFrame,
    ...
    private:
        ...
        static void loadGpuSensorCaptureSettings(const Settings& settings_json, CaptureSettingsMap& capture_settings)
        {
            CaptureSetting capture_setting;
            createSonarCaptureSettings(settings_json, capture_setting, ImageType::SegmentationID);
            capture_settings[capture_setting.image_type] = capture_setting;

            createSonarCaptureSettings(settings_json, capture_setting, ImageType::SurfaceNormals);
            capture_settings[capture_setting.image_type] = capture_setting;

            createSonarCaptureSettings(settings_json, capture_setting, ImageType::DepthPerspective);
            capture_settings[capture_setting.image_type] = capture_setting;
        }
                                         ...
                                         const std::map<std::string, std::shared_ptr<SensorSetting>>& sensor_defaults)
        {
            ...
            else if (simmode_name == kSimModeTypeAsv) {
                // create PhysX as default car vehicle
                auto asv_simple_setting = std::unique_ptr<VehicleSetting>(new VehicleSetting("AsvSimple", kVehicleTypeAsvSimple));
                asv_simple_setting->sensors = sensor_defaults;
                vehicles[asv_simple_setting->vehicle_name] = std::move(asv_simple_setting);
            }
            else if (simmode_name == kSimModeTypeAuv) {
                // create PhysX as default car vehicle
                auto auv_simple_setting = std::unique_ptr<VehicleSetting>(new VehicleSetting("AuvSimple", kVehicleTypeAuvSimple));
                auv_simple_setting->sensors = sensor_defaults;
                vehicles[auv_simple_setting->vehicle_name] = std::move(auv_simple_setting);
            }
            else if (simmode_name == kSimModeTypeComputerVision) {
        ...
        static void initializePawnPaths(std::map<std::string, PawnPath>& pawn_paths)
        {
            ...
            pawn_paths.emplace("DefaultAsv",
                               PawnPath("Class'/AirSim/Blueprints/BP_AsvPawn.BP_AsvPawn_C'"));
            pawn_paths.emplace("DefaultAuv",
                               PawnPath("Class'/AirSim/Blueprints/BP_AuvPawn.BP_AuvPawn_C'"));
        ...
        static CameraSetting createGpuSonarCameraSetting(const Settings& settings_json, const CameraSetting& camera_defaults)
        {
            CameraSetting setting = camera_defaults;
            ...
            loadGpuSensorCaptureSettings(settings_json, setting.capture_settings);
            loadNoiseSettings(settings_json, setting.noise_settings);
            Settings json_gimbal;
            if (settings_json.getChild("Gimbal", json_gimbal))
                setting.gimbal = createGimbalSetting(json_gimbal);
                                       ...
                                       const CameraSetting& camera_defaults)
        {
            ...
            // We don't call initializeCaptureSettings here since it doesn't work. I don't know why.
            // I figured out how to fix it. must call setCameraTypeEnabled and setCameraTypeUpdate.
            // gpu sensor cameras
            msr::airlib::Settings sensors_child;
            if (settings_json.getChild("Sensors", sensors_child)) {
                std::vector<std::string> sensor_keys;
                sensors_child.getChildNames(sensor_keys);

                for (const auto& key : sensor_keys) {
                    msr::airlib::Settings sensor_child;
                    sensors_child.getChild(key, sensor_child); // key->sensor name

                    auto sensor_type = Utils::toEnum<SensorBase::SensorType>(sensor_child.getInt("SensorType", 0));
                    auto enabled = sensor_child.getBool("Enabled", false);

                    if (sensor_type == SensorBase::SensorType::GpuSonar && enabled) {
                        cameras[key] = createGpuSonarCameraSetting(sensor_child, camera_defaults);
                    }
                    if (sensor_type == SensorBase::SensorType::GpuSidescanSonar && enabled) {
                        auto port_setting = createGpuSonarCameraSetting(sensor_child, camera_defaults);
                        Settings port_child;
                        ...
                        auto starboard_setting = createGpuSonarCameraSetting(sensor_child, camera_defaults);
                        Settings starboard_child;
        ...
        static void createSonarCaptureSettings(const msr::airlib::Settings& settings_json, CaptureSetting& capture_setting, const ImageType& image_type)
        {
            capture_setting.height = settings_json.getInt("NumberOfLaserSamples", 456);
            // Anamorphic rendering: width maps HorizontalRenderScale pixels per beam, decoupled from HFOV/VFOV ratio.
            // scale=1 is pure anamorphic (min GPU). Raise to 2/4/8 if low horizontal res causes post-process artifacts.
            const int horizontal_render_scale = std::max(1, settings_json.getInt("HorizontalRenderScale", 1));
            capture_setting.width = settings_json.getInt("NumberOfBeams", 96) * horizontal_render_scale;
            capture_setting.fov_degrees = settings_json.getFloat("HorizontalFOV", 29.0f);
            capture_setting.fov_vertical_degrees = settings_json.getFloat("VerticalFOV", 14.0f);
            capture_setting.auto_exposure_speed = settings_json.getFloat("AutoExposureSpeed", capture_setting.auto_exposure_speed);
            capture_setting.auto_exposure_bias = settings_json.getFloat("AutoExposureBias", capture_setting.auto_exposure_bias);
            capture_setting.auto_exposure_max_brightness = settings_json.getFloat("AutoExposureMaxBrightness", capture_setting.auto_exposure_max_brightness);
            capture_setting.auto_exposure_min_brightness = settings_json.getFloat("AutoExposureMinBrightness", capture_setting.auto_exposure_min_brightness);
            capture_setting.motion_blur_amount = settings_json.getFloat("MotionBlurAmount", capture_setting.motion_blur_amount);
            capture_setting.image_type = Utils::toNumeric(image_type);
            // sonar doesn't use target gamma, but we set it to a default value
            capture_setting.target_gamma = settings_json.getFloat("TargetGamma",
                                                                  capture_setting.image_type == 0 ? CaptureSetting::kSceneTargetGamma : Utils::nan<float>());

            capture_setting.projection_mode = 0; // Perspective
            // sonar doesn't use orthographic width, but we set it to a default value
            capture_setting.ortho_width = settings_json.getFloat("OrthoWidth", capture_setting.ortho_width);
        }
            ...
            bool enabled)
        {
            ...
            case SensorBase::SensorType::Pressure:
                sensor_setting = std::shared_ptr<SensorSetting>(new PressureSetting());
                break;
            ...
            case SensorBase::SensorType::GpuSonar:
                sensor_setting = std::shared_ptr<SensorSetting>(new GpuSonarSetting());
                break;
            case SensorBase::SensorType::GpuSidescanSonar:
                sensor_setting = std::shared_ptr<SensorSetting>(new GpuSidescanSonarSetting());
                break;
            default:
                                                ...
                                                std::map<std::string, std::shared_ptr<SensorSetting>>& sensors)
        {
            if (simmode_name == kSimModeTypeMultirotor) {
                ...
                sensors["imu"] = createSensorSetting(SensorBase::SensorType::Imu, "imu", true);
                sensors["pressure"] = createSensorSetting(SensorBase::SensorType::Pressure, "pressure", true);
                sensors["dvl"] = createSensorSetting(SensorBase::SensorType::Dvl, "dvl", true);
            }
            else {

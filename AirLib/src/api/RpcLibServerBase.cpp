...
namespace airlib
{
        ...
        : api_provider_(api_provider)
    {
        ...
        // pimpl_->server.bind("simPlayBackTrajectory", [&](const std::vector<RpcLibAdaptorsBase::Pose>& trajectory, std::vector<TTimePoint>& time_stamps, const std::string& vehicle_name) -> bool {
        //     getVehicleSimApi(vehicle_name)->playBack(trajectory, time_stamps);
        // });
        ...
        pimpl_->server.bind("getGpuSonarData", [&](const std::string& sonar_name, const std::string& vehicle_name) -> RpcLibAdaptorsBase::GpuSonarData {
            const auto& sonar_data = getVehicleApi(vehicle_name)->getGpuSonarData(sonar_name);
            return RpcLibAdaptorsBase::GpuSonarData(sonar_data);
        });

        pimpl_->server.bind("getGpuSidescanSonarData", [&](const std::string& sonar_name, const std::string& vehicle_name) -> RpcLibAdaptorsBase::GpuSidescanSonarData {
            const auto& sonar_data = getVehicleApi(vehicle_name)->getGpuSidescanSonarData(sonar_name);
            return RpcLibAdaptorsBase::GpuSidescanSonarData(sonar_data);
        });
        ...
        pimpl_->server.bind("getPressureData", [&](const std::string& pressure_name, const std::string& vehicle_name) -> RpcLibAdaptorsBase::PressureData {
            const auto& pressure_data = getVehicleApi(vehicle_name)->getPressureData(pressure_name);
            return RpcLibAdaptorsBase::PressureData(pressure_data);
        });
        ...
        pimpl_->server.bind("getDvlData", [&](const std::string& dvl_name, const std::string& vehicle_name) -> RpcLibAdaptorsBase::DvlData {
            const auto& dvl_data = getVehicleApi(vehicle_name)->getDvlData(dvl_name);
            return RpcLibAdaptorsBase::DvlData(dvl_data);
        });

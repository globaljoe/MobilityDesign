...
AirsimROSWrapper::AirsimROSWrapper(const ros::NodeHandle& nh, const ros::NodeHandle& nh_private, const std::string& host_ip)
    ...
    , gpu_sonar_async_spinner_(1, &gpu_sonar_timer_cb_queue_) // same as above, but for gpu sonar
    , gpu_sidescan_sonar_async_spinner_(1, &gpu_sidescan_sonar_timer_cb_queue_) // same as above, but for gpu sidescan sonar
    ...
    , tf_listener_(tf_buffer_)
{
    ...
    else if (AirSimSettings::singleton().simmode_name == AirSimSettings::kSimModeTypeAsv) {
        airsim_mode_ = AIRSIM_MODE::ASV;
        ROS_INFO("Setting ROS wrapper to ASV mode");
    }
    else if (AirSimSettings::singleton().simmode_name == AirSimSettings::kSimModeTypeAuv) {
        airsim_mode_ = AIRSIM_MODE::AUV;
        ROS_INFO("Setting ROS wrapper to AUV mode");
    }
    else {
...
void AirsimROSWrapper::initialize_airsim()
{
    ...
    try {
        ...
        else if (airsim_mode_ == AIRSIM_MODE::ASV){
            airsim_client_ = std::unique_ptr<msr::airlib::RpcLibClientBase>(new msr::airlib::AsvRpcLibClient(host_ip_));
        }
...
void AirsimROSWrapper::create_ros_pubs_from_settings_json()
{
    ...
    origin_geo_point_pub_ = nh_private_.advertise<furosim_ros_pkgs::GPSYaw>("origin_geo_point", 10);
    ...
    for (const auto& curr_vehicle_elem : AirSimSettings::singleton().vehicles) {
        ...
        if (airsim_mode_ == AIRSIM_MODE::DRONE) {
            ...
            vehicle_ros = std::unique_ptr<AsvROS>(new AsvROS());
        }
        else if (airsim_mode_ == AIRSIM_MODE::AUV) {
        ...
        vehicle_ros->env_pub = nh_private_.advertise<furosim_ros_pkgs::Environment>(curr_vehicle_name + "/environment", 10);
        ...
        if (airsim_mode_ == AIRSIM_MODE::DRONE) {
            ...
            drone->vel_cmd_body_frame_sub = nh_private_.subscribe<furosim_ros_pkgs::VelCmd>(
            ...
            drone->vel_cmd_world_frame_sub = nh_private_.subscribe<furosim_ros_pkgs::VelCmd>(
                curr_vehicle_name + "/vel_cmd_world_frame",
                1,
                boost::bind(&AirsimROSWrapper::vel_cmd_world_frame_cb, this, _1, vehicle_ros->vehicle_name));

            drone->takeoff_srvr = nh_private_.advertiseService<furosim_ros_pkgs::Takeoff::Request, furosim_ros_pkgs::Takeoff::Response>(
                curr_vehicle_name + "/takeoff",
                boost::bind(&AirsimROSWrapper::takeoff_srv_cb, this, _1, _2, vehicle_ros->vehicle_name));

            drone->land_srvr = nh_private_.advertiseService<furosim_ros_pkgs::Land::Request, furosim_ros_pkgs::Land::Response>(
            ...
            auto asv = static_cast<AsvROS*>(vehicle_ros.get());
            //----------------------------------------------//
        }
        else if (airsim_mode_ == AIRSIM_MODE::AUV) {
            auto auv = static_cast<AuvROS*>(vehicle_ros.get());
            auv->auv_cmd_sub = nh_private_.subscribe<furosim_ros_pkgs::VelCmd>(
                curr_vehicle_name + "/force_cmd",
                1,
                boost::bind(&AirsimROSWrapper::auv_cmd_cb, this, _1, vehicle_ros->vehicle_name));
            auv->ocean_current_sub = nh_private_.subscribe<geometry_msgs::Vector3Stamped>(
            ...
            car->car_cmd_sub = nh_private_.subscribe<furosim_ros_pkgs::CarControls>(
                curr_vehicle_name + "/car_cmd",
                1,
                boost::bind(&AirsimROSWrapper::car_cmd_cb, this, _1, vehicle_ros->vehicle_name));

            car->car_state_pub = nh_private_.advertise<furosim_ros_pkgs::CarState>(curr_vehicle_name + "/car_state", 10);
        }
        ...
        for (auto& curr_camera_elem : vehicle_setting->cameras) {
            ...
            for (const auto& curr_capture_elem : camera_setting.capture_settings) {
                ...
                // skip the special -1 (main camera) entry to avoid duplicate topic names
                if (curr_capture_elem.first < 0)
                    continue;
                ...
                if (!(std::isnan(capture_setting.fov_degrees))) {
                    ...
                    // if scene / segmentation / surface normals / infrared / OpticalFlow / OpticalFlowVis, get uncompressed image with pixels_as_floats = false
                    if (curr_image_type == ImageType::Scene ||
                        ...
                        curr_image_type == ImageType::OpticalFlow ||
                        curr_image_type == ImageType::OpticalFlowVis) {
        ...
        for (const auto& [sensor_name, sensor_setting] : vehicle_setting->sensors) {
            if (sensor_setting->enabled) {
                ...
                case SensorBase::SensorType::Barometer: {
                    ...
                    sensor_publisher.publisher = nh_private_.advertise<furosim_ros_pkgs::Altimeter>(curr_vehicle_name + "/altimeter/" + sensor_name, 10);
                    break;
                }
                case SensorBase::SensorType::Pressure: {
                    ROS_INFO_STREAM(sensor_name << ": Pressure sensor");
                    sensor_publisher.publisher = nh_private_.advertise<furosim_ros_pkgs::Pressure>(curr_vehicle_name + "/pressure/" + sensor_name, 10);
                    break;
                }
                ...
                case SensorBase::SensorType::Dvl: {
                    ROS_INFO_STREAM(sensor_name << ": Dvl sensor");
                    sensor_publisher.publisher = nh_private_.advertise<furosim_ros_pkgs::DVL>(curr_vehicle_name + "/dvl/" + sensor_name, 10);
                    break;
                }
                ...
                case SensorBase::SensorType::GpuSonar: {
                    ROS_INFO_STREAM(sensor_name << ": GpuSonar");
                    ...
                    sensor_publisher.publisher = nh_private_.advertise<sensor_msgs::Image>(curr_vehicle_name + "/gpu_sonar/" + sensor_name, 10);
                ...
                case SensorBase::SensorType::GpuSidescanSonar: {
                    ROS_INFO_STREAM(sensor_name << ": GpuSidescanSonar");
                    ...
                    sensor_publisher.publisher = nh_private_.advertise<sensor_msgs::Image>(curr_vehicle_name + "/gpu_sidescan_sonar/" + sensor_name, 10);
                    sensor_publisher.image_height = 1;
                    sensor_publisher.image_width = params.number_of_samples;
                    break;
                }
                default: {
        ...
        auto isGpuSonar = [](const SensorPublisher& pub) {
            return pub.sensor_type == SensorBase::SensorType::GpuSonar;
        };
        ...
        size_t cnt_gs = std::count_if(sensors.begin(), sensors.end(), isGpuSonar);
        gpu_sonar_cnt += cnt_gs;
        vehicle_ros->gpu_sonar_pubs.resize(cnt_gs);
        vehicle_ros->sensor_pubs.resize(sensors.size() - cnt_gs);
        std::partition_copy(sensors.begin(), sensors.end(), vehicle_ros->gpu_sonar_pubs.begin(), vehicle_ros->sensor_pubs.begin(), isGpuSonar);

        auto isGpuSidescanSonar = [](const SensorPublisher& pub) {
            return pub.sensor_type == SensorBase::SensorType::GpuSidescanSonar;
        };
        size_t cnt_gss = std::count_if(sensors.begin(), sensors.end(), isGpuSidescanSonar);
        gpu_sidescan_sonar_cnt += cnt_gss;
        vehicle_ros->gpu_sidescan_sonar_pubs.resize(cnt_gss);
        vehicle_ros->sensor_pubs.resize(sensors.size() - cnt_gss);
        std::partition_copy(sensors.begin(), sensors.end(), vehicle_ros->gpu_sidescan_sonar_pubs.begin(), vehicle_ros->sensor_pubs.begin(), isGpuSidescanSonar);
    ...
    // gpu sonars update on their own callback/thread at a given rate
    if (gpu_sonar_cnt > 0) {
        double update_gpu_sonar_every_n_sec;
        nh_private_.getParam("update_sonar_every_n_sec", update_gpu_sonar_every_n_sec);

        ros::TimerOptions timer_options(ros::Duration(update_gpu_sonar_every_n_sec),
                                        boost::bind(&AirsimROSWrapper::gpu_sonar_timer_cb, this, _1),
                                        &gpu_sonar_timer_cb_queue_);

        airsim_gpu_sonar_update_timer_ = nh_private_.createTimer(timer_options);
        is_used_gpu_sonar_timer_cb_queue_ = true;
    }

    // gpu sidescan sonars update on their own callback/thread at a given rate
    if (gpu_sidescan_sonar_cnt > 0) {
        double update_gpu_sidescan_sonar_every_n_sec;
        nh_private_.getParam("update_sidescan_sonar_every_n_sec", update_gpu_sidescan_sonar_every_n_sec);

        ros::TimerOptions timer_options(ros::Duration(update_gpu_sidescan_sonar_every_n_sec),
                                        boost::bind(&AirsimROSWrapper::gpu_sidescan_sonar_timer_cb, this, _1),
                                        &gpu_sidescan_sonar_timer_cb_queue_);

        airsim_gpu_sidescan_sonar_update_timer_ = nh_private_.createTimer(timer_options);
...
bool AirsimROSWrapper::takeoff_srv_cb(furosim_ros_pkgs::Takeoff::Request& request, furosim_ros_pkgs::Takeoff::Response& response, const std::string& vehicle_name)
{
...
bool AirsimROSWrapper::takeoff_group_srv_cb(furosim_ros_pkgs::TakeoffGroup::Request& request, furosim_ros_pkgs::TakeoffGroup::Response& response)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
bool AirsimROSWrapper::takeoff_all_srv_cb(furosim_ros_pkgs::Takeoff::Request& request, furosim_ros_pkgs::Takeoff::Response& response)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
bool AirsimROSWrapper::land_srv_cb(furosim_ros_pkgs::Land::Request& request, furosim_ros_pkgs::Land::Response& response, const std::string& vehicle_name)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
bool AirsimROSWrapper::land_group_srv_cb(furosim_ros_pkgs::LandGroup::Request& request, furosim_ros_pkgs::LandGroup::Response& response)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
bool AirsimROSWrapper::land_all_srv_cb(furosim_ros_pkgs::Land::Request& request, furosim_ros_pkgs::Land::Response& response)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
bool AirsimROSWrapper::reset_srv_cb(furosim_ros_pkgs::Reset::Request& request, furosim_ros_pkgs::Reset::Response& response)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
void AirsimROSWrapper::car_cmd_cb(const furosim_ros_pkgs::CarControls::ConstPtr& msg, const std::string& vehicle_name)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
void AirsimROSWrapper::auv_cmd_cb(const furosim_ros_pkgs::VelCmd::ConstPtr& msg, const std::string& vehicle_name)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);

    auto auv = static_cast<AuvROS*>(vehicle_name_ptr_map_[vehicle_name].get());

    auv->force_cmd.fx = msg->twist.linear.x;
    auv->force_cmd.fy = msg->twist.linear.y;
    auv->force_cmd.fz = msg->twist.linear.z;
    auv->force_cmd.torque_roll = msg->twist.angular.x;
    auv->force_cmd.torque_pitch = msg->twist.angular.y;
    auv->force_cmd.torque_yaw = msg->twist.angular.z;
    auv->has_force_cmd = true;
}

void AirsimROSWrapper::ocean_current_cb(const geometry_msgs::Vector3Stamped::ConstPtr& msg, const std::string& vehicle_name)
{
...
// void AirsimROSWrapper::vel_cmd_body_frame_cb(const furosim_ros_pkgs::VelCmd& msg, const std::string& vehicle_name)
void AirsimROSWrapper::vel_cmd_body_frame_cb(const furosim_ros_pkgs::VelCmd::ConstPtr& msg, const std::string& vehicle_name)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
void AirsimROSWrapper::vel_cmd_group_body_frame_cb(const furosim_ros_pkgs::VelCmdGroup& msg)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
// void AirsimROSWrapper::vel_cmd_all_body_frame_cb(const furosim_ros_pkgs::VelCmd::ConstPtr& msg)
void AirsimROSWrapper::vel_cmd_all_body_frame_cb(const furosim_ros_pkgs::VelCmd& msg)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
void AirsimROSWrapper::vel_cmd_world_frame_cb(const furosim_ros_pkgs::VelCmd::ConstPtr& msg, const std::string& vehicle_name)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
void AirsimROSWrapper::vel_cmd_group_world_frame_cb(const furosim_ros_pkgs::VelCmdGroup& msg)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
void AirsimROSWrapper::vel_cmd_all_world_frame_cb(const furosim_ros_pkgs::VelCmd& msg)
{
    std::lock_guard<std::mutex> guard(drone_control_mutex_);
...
void AirsimROSWrapper::gimbal_angle_quat_cmd_cb(const furosim_ros_pkgs::GimbalAngleQuatCmd& gimbal_angle_quat_cmd_msg)
{
...
void AirsimROSWrapper::gimbal_angle_euler_cmd_cb(const furosim_ros_pkgs::GimbalAngleEulerCmd& gimbal_angle_euler_cmd_msg)
{
    try {
        tf2::Quaternion quat_control_cmd;
...
furosim_ros_pkgs::CarState AirsimROSWrapper::get_roscarstate_msg_from_car_state(const msr::airlib::CarApiBase::CarState& car_state) const
{
    furosim_ros_pkgs::CarState state_msg;
...
nav_msgs::Odometry AirsimROSWrapper::get_odom_msg_from_asv_state(const msr::airlib::AsvApiBase::AsvState& asv_state) const
{
    nav_msgs::Odometry odom_msg;

    odom_msg.pose.pose.position.x = asv_state.getPosition().x();
    odom_msg.pose.pose.position.y = asv_state.getPosition().y();
    odom_msg.pose.pose.position.z = asv_state.getPosition().z();
    odom_msg.pose.pose.orientation.x = asv_state.getOrientation().x();
    odom_msg.pose.pose.orientation.y = asv_state.getOrientation().y();
    odom_msg.pose.pose.orientation.z = asv_state.getOrientation().z();
    odom_msg.pose.pose.orientation.w = asv_state.getOrientation().w();

    odom_msg.twist.twist.linear.x = asv_state.kinematics_estimated.twist.linear.x();
    odom_msg.twist.twist.linear.y = asv_state.kinematics_estimated.twist.linear.y();
    odom_msg.twist.twist.linear.z = asv_state.kinematics_estimated.twist.linear.z();
    odom_msg.twist.twist.angular.x = asv_state.kinematics_estimated.twist.angular.x();
    odom_msg.twist.twist.angular.y = asv_state.kinematics_estimated.twist.angular.y();
    odom_msg.twist.twist.angular.z = asv_state.kinematics_estimated.twist.angular.z();
...
nav_msgs::Odometry AirsimROSWrapper::get_odom_msg_from_auv_state(const msr::airlib::AuvApiBase::AuvState& auv_state) const
{
    nav_msgs::Odometry odom_msg;
...
sensor_msgs::ImagePtr AirsimROSWrapper::get_gpu_sonar_img_msg_from_airsim(const msr::airlib::GpuSonarData& sonar_data, const std::string& vehicle_name, const std::string& sensor_name, int image_height, int image_width) const
{
    sensor_msgs::ImagePtr sonar_img_msg;
...
sensor_msgs::ImagePtr AirsimROSWrapper::get_gpu_sidescan_sonar_img_msg_from_airsim(const msr::airlib::GpuSidescanSonarData& sonar_data, const std::string& vehicle_name, const std::string& sensor_name) const
{
    sensor_msgs::ImagePtr sonar_img_msg;
...
furosim_ros_pkgs::Environment AirsimROSWrapper::get_environment_msg_from_airsim(const msr::airlib::Environment::State& env_data) const
{
    furosim_ros_pkgs::Environment env_msg;
...
furosim_ros_pkgs::DVL AirsimROSWrapper::get_dvl_msg_from_airsim(const msr::airlib::DvlData& dvl_data) const
{
    furosim_ros_pkgs::DVL dvl_msg;
    dvl_msg.header.stamp = airsim_timestamp_to_ros(dvl_data.time_stamp);
...
furosim_ros_pkgs::Altimeter AirsimROSWrapper::get_altimeter_msg_from_airsim(const msr::airlib::BarometerBase::Output& alt_data) const
{
    furosim_ros_pkgs::Altimeter alt_msg;
...
furosim_ros_pkgs::Pressure AirsimROSWrapper::get_pressure_msg_from_airsim(const msr::airlib::PressureBase::Output& prs_data) const
{
    furosim_ros_pkgs::Pressure prs_msg;
    prs_msg.header.stamp = airsim_timestamp_to_ros(prs_data.time_stamp);
...
furosim_ros_pkgs::GPSYaw AirsimROSWrapper::get_gps_msg_from_airsim_geo_point(const msr::airlib::GeoPoint& geo_point) const
{
    furosim_ros_pkgs::GPSYaw gps_msg;
...
msr::airlib::AsvRpcLibClient* AirsimROSWrapper::get_asv_client()
{
    return static_cast<msr::airlib::AsvRpcLibClient*>(airsim_client_.get());
}
...
ros::Time AirsimROSWrapper::update_state()
{
    ...
    for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
        ...
        if (airsim_mode_ == AIRSIM_MODE::DRONE) {
            ...
            auto asv = static_cast<AsvROS*>(vehicle_ros.get());
            asv->curr_asv_state = get_asv_client()->getAsvState(vehicle_ros->vehicle_name);

            vehicle_time = airsim_timestamp_to_ros(asv->curr_asv_state.timestamp);
            if (!got_sim_time) {
                curr_ros_time = vehicle_time;
                got_sim_time = true;
            }
            ...
            vehicle_ros->curr_odom = get_odom_msg_from_asv_state(asv->curr_asv_state);

            // furosim_ros_pkgs::AsvState state_msg = get_rosasvstate_msg_from_asv_state(asv->curr_asv_state);
            // state_msg.header.frame_id = vehicle_ros->vehicle_name;
            // asv->asv_state_msg = state_msg;
        }
        else if (airsim_mode_ == AIRSIM_MODE::AUV) {
            auto auv = static_cast<AuvROS*>(vehicle_ros.get());
            auv->curr_auv_state = get_auv_client()->getAuvState(vehicle_ros->vehicle_name);

            vehicle_time = airsim_timestamp_to_ros(auv->curr_auv_state.timestamp);
            if (!got_sim_time) {
                curr_ros_time = vehicle_time;
                got_sim_time = true;
            }
            ...
            vehicle_ros->curr_odom = get_odom_msg_from_auv_state(auv->curr_auv_state);

            // furosim_ros_pkgs::AuvState state_msg = get_rosauvstate_msg_from_auv_state(auv->curr_auv_state);
            // state_msg.header.frame_id = vehicle_ros->vehicle_name;
            // auv->auv_state_msg = state_msg;
        }
        else {
            auto car = static_cast<CarROS*>(vehicle_ros.get());
            ...
            furosim_ros_pkgs::CarState state_msg = get_roscarstate_msg_from_car_state(car->curr_car_state);
            state_msg.header.frame_id = vehicle_ros->vehicle_name;
        ...
        furosim_ros_pkgs::Environment env_msg = get_environment_msg_from_airsim(env_data);
    ...
    for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
        ...
        for (auto& sensor_publisher : vehicle_ros->sensor_pubs) {
            ...
            case SensorBase::SensorType::Barometer: {
                ...
                furosim_ros_pkgs::Altimeter alt_msg = get_altimeter_msg_from_airsim(baro_data);
                alt_msg.header.frame_id = vehicle_ros->vehicle_name;
                sensor_publisher.publisher.publish(alt_msg);
                break;
            }
            case SensorBase::SensorType::Pressure: {
                auto prs_data = airsim_client_->getPressureData(sensor_publisher.sensor_name, vehicle_ros->vehicle_name);
                furosim_ros_pkgs::Pressure prs_msg = get_pressure_msg_from_airsim(prs_data);
                prs_msg.header.frame_id = vehicle_ros->vehicle_name;
            ...
            case SensorBase::SensorType::Dvl: {
                auto dvl_data = airsim_client_->getDvlData(sensor_publisher.sensor_name, vehicle_ros->vehicle_name);
                furosim_ros_pkgs::DVL dvl_msg = get_dvl_msg_from_airsim(dvl_data);
                dvl_msg.header.frame_id = vehicle_ros->vehicle_name;
...
void AirsimROSWrapper::update_commands()
{
    for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
        ...
        if (airsim_mode_ == AIRSIM_MODE::DRONE) {
            ...
            if (auv->has_force_cmd) {
                std::lock_guard<std::mutex> guard(drone_control_mutex_);
                get_auv_client()->setAuvControls(auv->force_cmd.fx,
                                                 auv->force_cmd.fy,
                                                 auv->force_cmd.fz,
                                                 auv->force_cmd.torque_roll,
...
{
    geometry_msgs::TransformStamped sonar_tf_msg;
    sonar_tf_msg.header.frame_id = vehicle_ros->vehicle_name + "/" + odom_frame_id_;
    sonar_tf_msg.child_frame_id = vehicle_ros->vehicle_name + "/" + sonar_name;
    sonar_tf_msg.transform.translation.x = sonar_setting.relative_pose.position.x();
    sonar_tf_msg.transform.translation.y = sonar_setting.relative_pose.position.y();
    sonar_tf_msg.transform.translation.z = sonar_setting.relative_pose.position.z();
    sonar_tf_msg.transform.rotation.x = sonar_setting.relative_pose.orientation.x();
    sonar_tf_msg.transform.rotation.y = sonar_setting.relative_pose.orientation.y();
    sonar_tf_msg.transform.rotation.z = sonar_setting.relative_pose.orientation.z();
    sonar_tf_msg.transform.rotation.w = sonar_setting.relative_pose.orientation.w();

    if (isENU_) {
        std::swap(sonar_tf_msg.transform.translation.x, sonar_tf_msg.transform.translation.y);
        std::swap(sonar_tf_msg.transform.rotation.x, sonar_tf_msg.transform.rotation.y);
        sonar_tf_msg.transform.translation.z = -sonar_tf_msg.transform.translation.z;
        sonar_tf_msg.transform.rotation.z = -sonar_tf_msg.transform.rotation.z;
    }

    vehicle_ros->static_tf_msg_vec.emplace_back(sonar_tf_msg);
}
...
void AirsimROSWrapper::gpu_sonar_timer_cb(const ros::TimerEvent& event)
{
    try {
        for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
            if (!vehicle_name_ptr_pair.second->gpu_sonar_pubs.empty()) {
                for (auto& sonar_publisher : vehicle_name_ptr_pair.second->gpu_sonar_pubs) {
                    auto sonar_data = airsim_client_gpu_sonar_.getGpuSonarData(sonar_publisher.sensor_name, vehicle_name_ptr_pair.first);
                    sensor_msgs::ImagePtr sonar_msg = get_gpu_sonar_img_msg_from_airsim(sonar_data, vehicle_name_ptr_pair.first, sonar_publisher.sensor_name, sonar_publisher.image_height, sonar_publisher.image_width);
                    if (sonar_msg) {
        ...
        std::string msg = e.get_error().as<std::string>();
        std::cout << "Exception raised by the API, didn't get sonar response." << std::endl
                  << msg << std::endl;
    }
}

void AirsimROSWrapper::gpu_sidescan_sonar_timer_cb(const ros::TimerEvent& event)
{
    try {
        for (auto& vehicle_name_ptr_pair : vehicle_name_ptr_map_) {
            if (!vehicle_name_ptr_pair.second->gpu_sidescan_sonar_pubs.empty()) {
                for (auto& sonar_publisher : vehicle_name_ptr_pair.second->gpu_sidescan_sonar_pubs) {
                    ...
                    sensor_msgs::ImagePtr sonar_msg = get_gpu_sidescan_sonar_img_msg_from_airsim(sonar_data, vehicle_name_ptr_pair.first, sonar_publisher.sensor_name);
                    if (sonar_msg) {
                        sonar_publisher.publisher.publish(sonar_msg);
                    }
                }
        ...
        std::string msg = e.get_error().as<std::string>();
        std::cout << "Exception raised by the API, didn't get sidescan sonar response." << std::endl
                  << msg << std::endl;
    }
}

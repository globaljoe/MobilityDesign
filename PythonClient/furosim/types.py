...
class AuvControls(MsgpackMixin):
    ...
    def __init__(self, fx=0.0, fy=0.0, fz=0.0, roll=0.0, pitch=0.0, yaw=0.0):
        self.force  = [fx, fy, fz]
        self.torque = [roll, pitch, yaw]
...
class GpuSonarData(MsgpackMixin):
    time_stamp = np.uint64(0)
    min_range = 0.0
    max_range = 0.0
    sonar_raw_data = 0.0
    segmentation_sonar = 0
    point_cloud = 0.0
    segmentation = 0
    pose = Pose()
...
class GpuSidescanSonarData(MsgpackMixin):
    time_stamp = np.uint64(0)
    min_range = 0.0
    max_range = 0.0
    port_raw_data = 0.0
    port_segmentation = 0
    port_point_cloud = 0.0
    port_segmentation_cloud = 0

    starboard_raw_data = 0.0
    starboard_segmentation = 0
    starboard_point_cloud = 0.0
    starboard_segmentation_cloud = 0
    pose = Pose()
    ...
    attribute_order = [
        ...
        ('max_range', float),
        ('port_raw_data', float),
        ('starboard_raw_data', float),
        ('port_segmentation', int),
        ('starboard_segmentation', int),
        ('port_point_cloud', float),
        ('starboard_point_cloud', float),
        ('port_segmentation_cloud', int),
        ('starboard_segmentation_cloud', int),
        ('pose', Pose)
    ]
...
class PressureData(MsgpackMixin):
    time_stamp = np.uint64(0)
    depth = Quaternionr()
    pressure = Vector3r()
